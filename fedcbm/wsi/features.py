...
class FeatureGenerator():
    ...
    # HIPT models removed - commented out
    #def _hipt4k(self):
    #    model = HIPT_4K()
    #    model.eval()
    ...
    # CTransPath removed - commented out
    #def _transpath(self):
    #    model = ctranspath()
    ...
    def _hibou_l(self):
        ...
        transform = T.Compose([
            ...
            T.ToTensor(),
            T.Normalize(mean=[0.7068, 0.5755, 0.7220], std=[0.1950, 0.2316, 0.1816]),
        ])
    ...
    def forward_pass(self, image):
        ...
        elif self.model_name == 'hoptimus1':
            with torch.autocast(device_type="cuda", dtype=torch.float16):
                with torch.inference_mode():
                    features = self.model(transform(image).unsqueeze(0).to("cuda"))
        
        elif self.model_name == "phikonv2":
            processor = AutoImageProcessor.from_pretrained("owkin/phikon-v2")
        ...
        elif self.model_name == 'hibou_l':
            #AutoImageProcessor.from_pretrained("histai/hibou-b",trust_remote_code=True)
            #hf_data = self.transform(images=image, return_tensors="pt").to(self.device)
            with torch.no_grad():
                features = self.model(image)
