...
class WSIParser:
    ...
    def extract_tile_at_coordinates(
        ...
        """
        Extract a tile at specific coordinates from WSI.
        
        param: x int x coordinate at level 0
        param: y int y coordinate at level 0  
        param: tile_size tuple of (width, height) for tile dimensions, defaults to self.tile_dims
        param: normalize bool whether to apply stain normalization
        return: patch ndarray patch
        """
        ...
        x_at_level = int(x / self._downsample)
        y_at_level = int(y / self._downsample)
    ...
    def extract_multiple_tiles_at_coordinates(
        ...
        """
        Extract multiple tiles at specific coordinates from WSI.
        
        param: coordinates list of (x, y) coordinate tuples at level 0
        param: tile_size tuple of (width, height) for tile dimensions, defaults to self.tile_dims
        param: normalize bool whether to apply stain normalization
        return: list of (coordinates, tile) tuples
        """
        tiles = []
    ...
    def save(
        ...
        for (x, y), tile in func:
            ...
            if label_dir:
                ...
                save_dir = os.path.join(tile_path, os.path.basename(tile_path)[:16])
